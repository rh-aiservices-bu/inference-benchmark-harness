...
Point `RUN` at the harness campaign root containing `state.json` and `config.json`. Standalone AIPerf exports do not have those files; share their native `profile_export_aiperf.json` with the command/version instead. A generated summary file can contain a partial report: check the exit status and listed issues before sharing.
...
- Matrix rows include their recorded question, change and serving-profile label. These describe the intended experiment, not independent proof of the deployed policy. Rate labels include arrival mode and the client concurrency cap; completed requests/s does not establish achieved arrival rate. The saved configuration hash identifies the config but is not revalidated by reporting.
- Progress counts checkpoint entries with readable complete summaries and distinct in-range repeat slots. Duplicate replacement attempts never count as independent repeats. Unaccepted observations may contain useful client measurements but have not passed whole-attempt/group validation. A successful peer from a rejected mixed group remains unaccepted. Reporting checks these structures, not the original evidence hashes.
...
- The streaming/workload table reads native `inter_token_latency.p95` (ms), `output_token_throughput.avg` (tokens/sec), `input_sequence_length.avg` and `output_sequence_length.avg` (tokens), and `benchmark_duration.avg` (sec). These are full-run statistics, never attributed to the shared-arrival subset. Missing values stay unknown; unexpected units or invalid values are listed in checks. Detailed JSON includes these optional values under `native_metrics`, keyed by attempt and stream.
- Missing, malformed or unreadable files are listed while other results remain visible. Artifact paths outside the run directory are refused. Missing measurements stay unknown. Empty/incomplete checkpointed mixed groups, missing shared-arrival summaries and complete checkpoints with missing repeats are flagged. A changing checkpoint is flagged; regenerate after writes finish. Reporting does not lock a live run, revalidate evidence hashes, infer process liveness or change runner acceptance.
- Goal rows retain each metric, threshold, observed range and evaluation counts by population. Latency thresholds use ms; `max_error_fraction` uses failed/total requests. No configured goal means no goal judgment. A miss is a valid negative result, not a reason to discard a repeat.
...
Use the listed checks to diagnose missing evidence before tuning. The report gives a status-based next action; it does not compare separate campaigns, choose detector limits, calculate cache-hit rates or server-side queue statistics, or query New Relic. Requested token shape is configuration; observed mean lengths come from native AIPerf exports. Use the [metric reference](metrics.md) to collect serving evidence over the same run windows.

| Decision | What the report supplies | What to check alongside it |
|---|---|---|
| Flow-control overhead below saturation | Per-run TTFT/request/ITL p95 ranges, completed requests/s, failures and token lengths | Matched off/on repeats, fixed cache/topology/workload, an agreed overhead tolerance, and serving evidence that admission was not limiting traffic |
| Detector comparison | Stage/profile labels, load, goal thresholds and client outcomes | Effective detector settings, saturation relative to its ceiling, admission queue size/duration/outcomes and engine pressure in the same window |
| Priority protection and lower-priority progress | Per-workload latency, completions and throughput | Matched simultaneous demand and time-resolved completions/queues; run-wide averages cannot establish absence of starvation |
