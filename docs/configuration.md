...
| `FORMAT` / CLI `--format` | Make: `auto` | Verify: readable terminal output, JSON when redirected. Report: compact Markdown-compatible text, including when redirected. Report accepts `text`, `markdown` or detailed `json`; direct `python -m bench report` defaults to JSON for existing callers. Verify accepts `text` or `json`; terminal colors respect `NO_COLOR` |
...
`report` takes `--run` and optional `--format`; `matrix-pause` takes only `--run`. `plan` sends no requests; `verify` makes read requests but no inference; run commands send inference. [Run and recovery behavior](operator-guide.md#when-the-runner-moves-on).
